...
class TestOrderTracker:
    async def test_register_and_get_order(
        self, tracker: OrderTracker, order1: Order
    ) -> None:
    ...
    async def test_get_by_decision(
        self, tracker: OrderTracker, order1: Order
    ) -> None:
        lifecycle = OrderLifecycleTracker(order1)
        await tracker.register(order1, lifecycle)
    ...
    async def test_get_by_symbol(
        self, tracker: OrderTracker, order1: Order, order2: Order
    ) -> None:
    ...
    async def test_update_order(
        ...
        await tracker.register(order1, lifecycle)
        updated = Order(
            **{**order1.__dict__, "status": OrderStatus.FILLED}
        )
