...
class ValidationResult:
    ...
    def __await__(self) -> Generator[Any, None, ValidationResult]:
        async def _coro() -> ValidationResult:
            return self
...
class OrderValidatorConfig:
    ...
    def __init__(
        ...
        eff_max_vol = max_quantity if max_quantity is not None else max_volume
        object.__setattr__(self, "min_volume", min_volume)
        object.__setattr__(self, "max_volume", eff_max_vol)
        object.__setattr__(self, "max_price", max_price)
        object.__setattr__(self, "min_price", min_price)
        object.__setattr__(self, "max_slippage_pips", max_slippage_pips)
        object.__setattr__(self, "require_stop_loss", require_stop_loss)
        object.__setattr__(self, "require_take_profit", require_take_profit)
        object.__setattr__(self, "max_position_size", max_position_size)
        object.__setattr__(
            self, "precision_check_enabled", precision_check_enabled
        )
        object.__setattr__(
            self, "market_status_check_enabled", market_status_check_enabled
        )
        object.__setattr__(
            self,
            "broker_availability_check_enabled",
            broker_availability_check_enabled,
        )
        object.__setattr__(self, "max_quantity", eff_max_vol)
    ...
    def validate(
        ...
        if order.quantity is not None:
            ...
            if order.quantity < self._config.min_volume:
                errors.append(
                    f"Quantity {order.quantity} below minimum {self._config.min_volume}"
                )
        ...
        # 4. Stop loss checks
        if self._config.require_stop_loss and order.stop_price is None and order.stop_loss is None:
            errors.append("Stop loss is required but not set")

        # 5. Take profit checks
        if self._config.require_take_profit and order.take_profit is None and order.price is None:
        ...
        if (
            ...
            and current_spread > self._config.max_slippage_pips
        ):
            warnings.append(
